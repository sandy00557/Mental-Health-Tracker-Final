...
import { Quotes } from "../data/QuotesData.js";
import React, { useState, useEffect } from "react";
import "../assets/animations/EmojisSection.css"; // ✅ NEW
...
const EmojisSection = () => {
  ...
  const emojis = [
    { emojiz: "😀", description: "Happy" },
    { emojiz: "😢", description: "Sad" },
    { emojiz: "😠", description: "Angry" },
    { emojiz: "😣", description: "Frustrated" },
    { emojiz: "😭", description: "Overwhelmed/Crying" },
  ];
  ...
  const handleSaveMood = () => {
    if (selectedMood) {
      ...
      localStorage.setItem(`isMoodSaved_${todayKey}`, true);
      localStorage.setItem(`savedTime_${todayKey}`, timestamp);
  ...
  //step d: save quote index when emoji clicked
  useEffect(() => {
    localStorage.setItem(
      `quoteMap_${todayKey}`,
      JSON.stringify(moodQuoteIndexMap)
  ...
  //step e: load stored data on date change
  useEffect(() => {
    const savedMap = localStorage.getItem(`quoteMap_${todayKey}`);
    const mood = localStorage.getItem(`selectedMood_${todayKey}`);
    const saved = localStorage.getItem(`isMoodSaved_${todayKey}`);
    ...
    if (savedMap) setMoodQuoteIndexMap(JSON.parse(savedMap));
    if (mood) setSelectedMood(mood);
    if (saved) setIsMoodSaved(JSON.parse(saved));
    if (time) setSavedTime(time);
  ...
  const handleMoodClick = (mood) => {
    ...
    const quotesLength = moodQuotes?.message.length || 0;
    const currentIndex = quoteTracker[mood] || 0;
    const nextIndex = (currentIndex + 1) % quotesLength;
  ...
  const getCurrentQuote = () => {
    ...
    const index = quoteTracker[selectedMood] || 0;
    return moodQuotes?.message[index] || "No quote available for this mood";
  };
  ...
  return (
    <>
      ...
      <div className="emoji-section-container">
        <div className="emoji-bg"></div> {/* floating particles bg */}
        <h3 className="emoji-title">😊 Emojis Section</h3>
        <p className="emoji-subtitle">
          <span>Use the emojis to express your current mental state</span>
        </p>
        ...
        {selectedMood && (
          <div className="emoji-quote-box">
            <h4>
              <span>{selectedMood} Quote:</span>
            </h4>
