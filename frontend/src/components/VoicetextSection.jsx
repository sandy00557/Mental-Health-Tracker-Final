...
const VoiceToText = () => {
  ...
  const recognitionRef = useRef(null);
  const transcriptRef = useRef(""); // ✅ live transcript holder
  const navigate = useNavigate();
  ...
  useEffect(() => {
    ...
    recognition.onresult = (event) => {
      ...
      if (finalTranscript) {
        transcriptRef.current += " " + finalTranscript; // ✅ update instantly
    ...
    recognition.onend = () => {
      ...
      if (transcriptRef.current.trim()) {
        detectMoodAI(transcriptRef.current);
      } else {
        console.warn("⚠️ No transcript to analyze.");
      }
    };
  ...
  const detectMoodAI = async (text) => {
    ...
    try {
      ...
      if (response.data.mood) {
        setMood(response.data.mood);
        console.log("🎭 Mood updated to:", response.data.mood);
      } else {
        console.warn("⚠️ No mood in API response:", response.data);
      }
    } catch (err) {
      console.error("❌ Mood detection failed:", err);
    }
